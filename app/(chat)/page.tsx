...
export default function Home() {
  ...
  const lastMessageIsEmpty = !lastMessage?.parts.some(
    (p) => p.type === "text" && p.text.length > 0,
  );
  const showLoading =
    status === "submitted" ||
    (status === "streaming" && lastMessageIsAI && lastMessageIsEmpty);
  ...
  useEffect(() => {
    ...
    if (lastMessage?.role === "user") {
      ...
      setTimeout(() => {
        const userMsgElement = document.getElementById(
        ...
        if (userMsgElement) {
          ...
          const calculatedHeight =
            windowHeight - navbarHeight - fixedPadding - userMsgHeight;
      ...
      <main className="flex items-center justify-center min-h-dvh px-4 pt-14">
        <div className="pt-8 pb-54 max-w-3xl w-full flex flex-col min-h-[calc(100dvh-3.5rem)]">
          ...
          {showLoading && (
            <div
              style={{
                minHeight: lastAiMinHeight ? `${lastAiMinHeight}px` : undefined,
              }}
            >
        ...
        <Input
          className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-3xl"
          onSend={handleSend}
        />
      </main>
    </div>
